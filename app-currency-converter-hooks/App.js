...
export default function App() {
  ...
  const [moedaSelecionda, setMoedaSelecionada] = useState(null);
  ...
  async function converter(){
    if(moedaBValor === 0 || moedaBValor === "" || moedaSelecionda === null){
      return;
    }

    const response = await api.get(`/all/${moedaSelecionda}-BRL`);
    console.log(response.data[moedaSelecionda].ask)

    let resultado = (response.data[moedaSelecionda].ask * parseFloat(moedaBValor));
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.areaMoeda}>
          ...
          <PickerItem
          moedas={moedas}
          moedaSelecionda={moedaSelecionda}
